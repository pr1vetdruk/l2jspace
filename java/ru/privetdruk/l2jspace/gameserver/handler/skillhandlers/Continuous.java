    ...
    public void useSkill(Creature activeChar, L2Skill skill, WorldObject[] targets) {
        ...
        for (WorldObject obj : targets) {
            ...
            if (skill.isOffensive() || skill.isDebuff()) {
                shieldDefense = Formula.calcShieldUse(activeChar, target, skill, false);
                acted = Formula.calcSkillSuccess(activeChar, target, skill, shieldDefense, bsps);
            }
            ...
            if (acted) {
                ...
                if (target instanceof Player && ((Player) target).isInDuel() && (skill.getSkillType() == SkillType.DEBUFF || skill.getSkillType() == SkillType.BUFF) && player != null && player.getDuelId() == ((Player) target).getDuelId()) {
                    for (AbstractEffect buff : skill.getEffects(activeChar, target, shieldDefense, bsps)) {
                        if (buff != null) {
